...
class DashboardPage(QWidget):
    def __init__(self, server_controller, clients_controller, logs_controller=None, parent=None):
        ...
        self.server_controller, self.clients_controller, self.logs_controller = server_controller, clients_controller, logs_controller
        self.start_time, self.x_data, self.y_data, self.ptr, self.last_packets = None, list(range(20)), [0]*20, 19, 0
    ...
    def _build_server_card(self):
        ...
        header = QHBoxLayout()
        header.addWidget(QLabel("Server Control", objectName="SectionHeader"))
        header.addStretch()
        ...
        health = QGridLayout()
        self.lbl_total_packets = self._add_stat(health, 0, 0, "Total Packets")
        self.lbl_loss = self._add_stat(health, 0, 1, "Global Loss")
        self.lbl_latency = self._add_stat(health, 1, 0, "Avg Latency")
        self.lbl_throughput = self._add_stat(health, 1, 1, "Throughput")
    ...
    def _add_stat(self, grid, row, col, title):
        ...
        lbl = QLabel("-", objectName="StatValue")
    ...
    def _build_graph_card(self):
        ...
        fill = QColor("#3A7FF9")
        fill.setAlpha(40)
        self.curve = self.plot_widget.plot(self.x_data, self.y_data, pen=pg.mkPen('#3A7FF9', width=3), fillLevel=0, brush=pg.mkBrush(fill))
    ...
    def _on_server_status(self, running, message):
        ...
        if running:
            if not self.start_time:
                ...
                self.clients_controller.reset_session()
            if not self.uptime_timer.isActive(): self.uptime_timer.start()
            self.x_data, self.y_data, self.ptr, self.last_packets = list(range(20)), [0]*20, 19, 0
            self.curve.setData(self.x_data, self.y_data)
            if not self.timer.isActive(): self.timer.start(1000)
        else:
    ...
    def _update_uptime(self):
        ...
        self.lbl_uptime.setText(f"{s//3600:02d}:{(s%3600)//60:02d}:{s%60:02d}")

    def _update_graph(self):
        packets, gaps, latency, count, size, size_count = 0, 0, 0.0, 0, 0, 0
        for c in self.clients_controller.get_clients():
            packets += c.get("packets_sent", 0)
            gaps += c.get("gaps", 0)
            latency += c.get("avg_latency") or 0
            count += 1 if c.get("avg_latency") else 0
            size += c.get("avg_packet_size") or 0
            size_count += 1 if c.get("avg_packet_size") else 0
    ...
    def _refresh_clients(self, clients):
        ...
        if table.rowCount() != len(clients): table.setRowCount(len(clients))
        for row, client in enumerate(clients):
            did, last, pkts = client.get("device_id") or "Unknown", str(client.get("last_seen") or "-"), int(client.get("packets_sent", 0))
            dups, gaps = int(client.get("duplicates", 0)), int(client.get("gaps", 0))
            lat = f"{client.get('avg_latency'):.1f} ms" if client.get("avg_latency") else "-"
            loss = f"{(gaps/(pkts+gaps)*100):.1f}%" if pkts+gaps > 0 else "0.0%"
            for col, txt in enumerate([did, last, lat, loss, str(pkts)]):
                item = table.item(row, col)
                if not item: table.setItem(row, col, QTableWidgetItem(txt))
                elif item.text() != txt: item.setText(txt)
            online = client.get("is_online", False)
            if not online: txt, clr = "● Offline", QColor("#94A3B8")
            elif dups == 0 and gaps == 0: txt, clr = "● Online", QColor("#10B981")
            elif gaps > 0: txt, clr = f"● {gaps} Lost", QColor("#EF4444")
            else: txt, clr = f"● {dups} Dups", QColor("#F59E0B")
            si = table.item(row, 5)
            if not si: si = QTableWidgetItem(txt); table.setItem(row, 5, si)
            else: si.setText(txt)
            si.setForeground(clr)
