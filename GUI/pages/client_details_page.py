...
class ClientDetailsPage(QWidget):
    ...
    def __init__(self, logs_controller=None, parent=None):
        ...
        self.logs_controller, self.client_data = logs_controller, {}
    ...
    def set_client(self, client_data):
        ...
        batching_text = f"Enabled (size: {batch_size})" if batch_size and str(batch_size) != "1" else "Disabled"
    ...
    def _update_table(self):
        ...
        for row, log in enumerate(logs):
            status = "Duplicate" if log.get("duplicate") else "Gap" if log.get("gap") else "OK"
    ...
    def _update_graphs(self):
        ...
        is_online = logs and logs[-1].get("arrival_dt") and (datetime.datetime.now() - logs[-1]["arrival_dt"]).total_seconds() < 30
