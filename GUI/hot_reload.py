...
class RH(FileSystemEventHandler):
    def __init__(self, p): self.p, self.pr = p, None; self.sp()
    def sp(self):
        if self.pr: self.pr.terminate(); self.pr.wait()
        print(f"Starting {self.p}..."); self.pr = subprocess.Popen([sys.executable, self.p])
...
def main():
    cd = os.path.dirname(os.path.abspath(__file__)); s = os.path.join(cd, "main.py")
    if not os.path.exists(s): print(f"Error: No {s}"); return
    h = RH(s); o = Observer(); o.schedule(h, cd, recursive=True); o.start(); print(f"Watching {cd}...")
    try:
