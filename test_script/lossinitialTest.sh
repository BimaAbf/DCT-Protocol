...
log_step "Starting 5% loss test run"
...
log_step "Applying netem loss 5%"
tmux new-session -d -s "$NETEM_SESSION"
tmux pipe-pane -o -t "$NETEM_SESSION" "cat >> '$NETEM_LOG'"
tmux send-keys -t "$NETEM_SESSION" "tc qdisc replace dev lo root netem loss 5%" C-m
tmux send-keys -t "$NETEM_SESSION" "echo 'Applied netem loss 5% on lo'; tail -f /dev/null" C-m
